...
int main(void)
{
  ...
  while (1)
  {
	  while(contador <= 99)
	  {
		  ...
		  decenas = contador % 100/10;

		  for(uint8_t i = 0; i < 30;i++)
		  {
			  HAL_GPIO_WritePin(GPIOB, Disp1_Pin, GPIO_PIN_SET);
			  HAL_GPIO_WritePin(GPIOB, Disp2_Pin, GPIO_PIN_RESET);
			  Display_Port(display[unidades]);
			  HAL_Delay(4);

			  HAL_GPIO_WritePin(GPIOB, Disp1_Pin, GPIO_PIN_RESET);
			  HAL_GPIO_WritePin(GPIOB, Disp2_Pin, GPIO_PIN_SET);
			  Display_Port(display[decenas]);
			  HAL_Delay(4);
		  }
		  contador++;
	  }
